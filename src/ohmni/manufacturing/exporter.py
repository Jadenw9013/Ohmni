...
class KiCadFabricationExporter:
    ...
    def export(self,pcb,drc,manufacturing,profile,destination):
        ...
        manifest=destination/"ohmni-fabrication-manifest.json";manifest_bytes=(json.dumps(portable,indent=2,sort_keys=True)+"\n").encode("utf-8");manifest.write_bytes(manifest_bytes)
        manifest_file=FabricationFile(relative_path=manifest.name,sha256=hashlib.sha256(manifest_bytes).hexdigest(),size_bytes=len(manifest_bytes),kind="Manifest")
        events.extend([_event(pcb.circuit_content_hash,EventKind.FABRICATION_PACKAGE_COMPLETED,"Fabrication package completed",{"package_fingerprint":package_hash,"file_count":len(files)+1}),_event(pcb.circuit_content_hash,EventKind.RELEASE_READY_FOR_REVIEW,"Release ready for manufacturing review",{"package_fingerprint":package_hash})])
